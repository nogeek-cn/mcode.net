...
export default createContentLoader(['!(.vitepress|public|images|.guthub|components|snippets|else-exclude|sh)/**/!(Vuex源码解析|Vue静态资源处理|index|README|TODO).md'], {
  includeSrc: true,
  async transform(data) {
    const promises: Promise<any>[] = []

    console.log("")
    console.log("url.list.foreach");

    data.forEach((item) => {
      // 用页面的一级标题作为文章标题（因为sidebar中可能是精简的标题）
      let title = item.src?.match(/^#\s+(.*)[\n\r][\s\S]*/)?.[1] || path.basename(item.url).replace(path.extname(item.url), '')
      // 标题可能用到了变量，需要替换
      ...
      console.log('https://mcode.net/' + encodeURI(link))

      let filepath = link.replace(/\.html$/i, '.md');
      // 这里兼容 cleanUrls 模式
      ...
      // 获取发布时间
      const task = getGitTimestamp(filepath).then((fileTimeInfo) => ({
        title,
        details: item.src
          // 去除html标签
          ?.replace(/<[^>]+?>/g, '')
